      ...
      {tab === "contacts" && (() => {
        ...
        return (<>
          <div style={{display:"flex",gap:8,alignItems:"center",padding:"12px 0 8px",flexWrap:"wrap"}}>
            ...
            {overdueN > 0 && <button onClick={()=>{setCtFocusMode(false);setCtDueFilter(f=>f==="overdue"?"":"overdue");}} style={{...}}>{overdueN} Overdue</button>}
            {dueTodayN > 0 && <button onClick={()=>{setCtFocusMode(false);setCtDueFilter(f=>f==="today"?"":"today");}} style={{...}}>{dueTodayN} Due today</button>}
            {dueSoonN > 0 && <button onClick={()=>{setCtFocusMode(false);setCtDueFilter(f=>f==="3days"?"":"3days");}} style={{...}}>{dueSoonN} Due soon</button>}
            {upcomingN > 0 && <button onClick={()=>{setCtFocusMode(false);setCtDueFilter(f=>f==="7days"?"":"7days");}} style={{...}}>{upcomingN} Upcoming</button>}
            {doneN > 0 && <button onClick={()=>{setCtFocusMode(false);setCtDueFilter(f=>f==="none"?"":"none");}} style={{...}}>{doneN} Completed</button>}
